...
# Embedded file name: toontown.toon.DistributedToonAI
...
import InventoryBase, Experience
...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def handleLogicalZoneChange(self, newZoneId, oldZoneId):
        ...
        if newZoneId == ToontownGlobals.GoofySpeedway:
            if self.hasTeleportAccess(ToontownGlobals.GoofySpeedway):
                self.addTeleportAccess(newZoneId)
        if simbase.wantPets:
            isInEstate = self.isInEstate()
            wasInEstate = self.wasInEstate()
            if isInEstate or wasInEstate:
    ...
    def announceZoneChange(self, newZoneId, oldZoneId):
        ...
        PetObserve.send(broadcastZones, PetObserve.PetActionObserve(PetObserve.Actions.CHANGE_ZONE, self.doId, (
         oldZoneId, newZoneId)))
        ...
        if lengthMatch != 0:
            if len(inventoryNetString) == 42:
                ...
                if len(inventoryNetString) == 49:
                    ...
                    oldLevels = 0
            if oldTracks == 0:
                oldLevels == 0 and self.notify.warning('reseting invalid inventory to MAX on toon: %s' % self.doId)
    ...
    def attemptAddNPCFriend(self, npcFriend, numCalls=1):
        ...
        if self.NPCFriendsDict.has_key(npcFriend):
            ...
            if npcFriends.has_key(npcFriend):
    ...
    def addToClothesTopsList(self, topTex, topTexColor, sleeveTex, sleeveTexColor):
        ...
        for i in range(0, len(self.clothesTopsList), 4):
            if self.clothesTopsList[i] == topTex:
                return self.clothesTopsList[(i + 1)] == topTexColor and self.clothesTopsList[(i + 2)] == sleeveTex and self.clothesTopsList[(i + 3)] == sleeveTexColor and 0
    ...
    def replaceItemInClothesTopsList(self, topTexA, topTexColorA, sleeveTexA, sleeveTexColorA, topTexB, topTexColorB, sleeveTexB, sleeveTexColorB):
        index = 0
        for i in range(0, len(self.clothesTopsList), 4):
            if self.clothesTopsList[i] == topTexA:
                self.clothesTopsList[i] = self.clothesTopsList[(i + 1)] == topTexColorA and self.clothesTopsList[(i + 2)] == sleeveTexA and self.clothesTopsList[(i + 3)] == sleeveTexColorA and topTexB
    ...
    def removeItemInClothesTopsList(self, topTex, topTexColor, sleeveTex, sleeveTexColor):
        ...
        for i in range(0, listLen, 4):
            if self.clothesTopsList[i] == topTex:
                self.clothesTopsList = self.clothesTopsList[(i + 1)] == topTexColor and self.clothesTopsList[(i + 2)] == sleeveTex and self.clothesTopsList[(i + 3)] == sleeveTexColor and self.clothesTopsList[0:i] + self.clothesTopsList[i + 4:listLen]
                return 1
    ...
    def addToClothesBottomsList(self, botTex, botTexColor):
        ...
        for i in range(0, len(self.clothesBottomsList), 2):
            if self.clothesBottomsList[i] == botTex:
                return self.clothesBottomsList[(i + 1)] == botTexColor and 0
    ...
    def replaceItemInClothesBottomsList(self, botTexA, botTexColorA, botTexB, botTexColorB):
        index = 0
        for i in range(0, len(self.clothesBottomsList), 2):
            if self.clothesBottomsList[i] == botTexA:
                self.clothesBottomsList[i] = self.clothesBottomsList[(i + 1)] == botTexColorA and botTexB
    ...
    def removeItemInClothesBottomsList(self, botTex, botTexColor):
        ...
        for i in range(0, len(self.clothesBottomsList), 2):
            if self.clothesBottomsList[i] == botTex:
                self.clothesBottomsList = self.clothesBottomsList[(i + 1)] == botTexColor and self.clothesBottomsList[0:i] + self.clothesBottomsList[i + 2:listLen]
                return 1
    ...
    def takeDamage(self, hpLost, quietly=0, sendTotal=1):
        if self.immortalMode:
            if quietly:
                self.sendUpdate('takeDamage', [hpLost])
            if hpLost > 0:
                if self.hp > 0:
                    self.hp -= hpLost
                    self.hp = self.hp <= 0 and -1
        if self.hpOwnedByBattle:
    ...
    def setTutorialAck(self, tutorialAck):
        if hasattr(self, 'tutorialAck') and self.tutorialAck:
            if tutorialAck:
                self.air.writeServerEvent('finishedTutorial', self.doId, '')
    ...
    def getQuest(self, id, visitNpcId=None):
        for quest in self.quests:
            if quest[0] == id:
                if visitNpcId:
                    if visitNpcId == quest[1] or visitNpcId == quest[2]:
                        return quest
                else:
                    return quest
    ...
    def hasTrackAccess(self, track):
        if self.trackArray:
            return track < len(self.trackArray) and self.trackArray[track]
        else:
            return 0
    ...
    def fixTrackAccess(self):
        ...
        if self.rewardTier in [0, 1, 2, 3]:
            ...
            if self.rewardTier in [4, 5, 6]:
                if numTracks != 3:
                    self.notify.warning('bad num tracks in tier: %s, %s' % (self.rewardTier, self.trackArray))
                    if self.trackArray[ToontownBattleGlobals.SOUND_TRACK]:
                        not self.trackArray[ToontownBattleGlobals.HEAL_TRACK] and self.b_setTrackAccess([0, 0, 0, 1, 1, 1, 0])
                    else:
                        if self.trackArray[ToontownBattleGlobals.HEAL_TRACK]:
                            not self.trackArray[ToontownBattleGlobals.SOUND_TRACK] and self.b_setTrackAccess([1, 0, 0, 0, 1, 1, 0])
                        else:
                            if soundExp >= healExp:
                ...
                if self.rewardTier in [7, 8, 9, 10]:
                    if numTracks != 4:
                        self.notify.warning('bad num tracks in tier: %s, %s' % (self.rewardTier, self.trackArray))
                        if self.trackArray[ToontownBattleGlobals.SOUND_TRACK]:
                            if not self.trackArray[ToontownBattleGlobals.HEAL_TRACK]:
                                dropExp >= lureExp and self.b_setTrackAccess([0, 0, 0, 1, 1, 1, 1])
                            else:
                                self.b_setTrackAccess([0, 0, 1, 1, 1, 1, 0])
                        else:
                            if self.trackArray[ToontownBattleGlobals.HEAL_TRACK]:
                                if not self.trackArray[ToontownBattleGlobals.SOUND_TRACK]:
                                    dropExp >= lureExp and self.b_setTrackAccess([1, 0, 0, 0, 1, 1, 1])
                                else:
                    ...
                    self.rewardTier in [11, 12, 13] and numTracks != 5 and self.notify.warning('bad num tracks in tier: %s, %s' % (self.rewardTier, self.trackArray))
                    if self.trackArray[ToontownBattleGlobals.SOUND_TRACK]:
                        if not self.trackArray[ToontownBattleGlobals.HEAL_TRACK]:
                            if self.trackArray[ToontownBattleGlobals.DROP_TRACK]:
                                if not self.trackArray[ToontownBattleGlobals.LURE_TRACK]:
                                    if healExp >= trapExp:
                                ...
                                if self.trackArray[ToontownBattleGlobals.HEAL_TRACK]:
                                    if not self.trackArray[ToontownBattleGlobals.SOUND_TRACK]:
                                        if self.trackArray[ToontownBattleGlobals.DROP_TRACK] and not self.trackArray[ToontownBattleGlobals.LURE_TRACK]:
                                            soundExp >= trapExp and self.b_setTrackAccess([1, 0, 0, 1, 1, 1, 1])
                                        else:
                        ...
                        if numTracks != 6:
                            self.notify.warning('bad num tracks in tier: %s, %s' % (self.rewardTier, self.trackArray))
                            sortedExp = [
                             healExp, trapExp, lureExp, soundExp, dropExp]
                            sortedExp.sort()
    ...
    def fixAvatar(self):
        ...
        if len(self.quests) > self.questCarryLimit:
            ...
            self.fixAvatar()
            anyChanged = 1
        if self.emoteAccess[0] and self.emoteAccess[1] and self.emoteAccess[2] and self.emoteAccess[3] and self.emoteAccess[4]:
    ...
    def setDeliverySchedule(self, onOrder, doUpdateLater=True):
        ...
        if hasattr(self, 'name'):
            if doUpdateLater:
                if self.air.doLiveUpdates and hasattr(self, 'air'):
                    taskName = self.uniqueName('next-delivery')
                    taskMgr.remove(taskName)
                    ...
                    if nextItem != None:
                        pass
                    duration = nextTime != None and max(10.0, nextTime * 60 - time.time())
    ...
    def setBothSchedules(self, onOrder, onGiftOrder, doUpdateLater=True):
        ...
        if not hasattr(self, 'air') or self.air == None:
            return
        taskName = doUpdateLater and self.air.doLiveUpdates and hasattr(self, 'name') and 'next-bothDelivery-%s' % self.doId
        now = int(time.time() / 60 + 0.5)
        nextItem = None
        nextGiftItem = None
        nextTime = None
        ...
        if self.houseId:
            if hasattr(self, 'air'):
                if self.air:
                    ...
                    house and house.mailbox and house.mailbox.b_setFullIndicator(len(mailboxContents) != 0)
    ...
    def takeMoney(self, deltaMoney, bUseBank=True):
        ...
        if deltaMoney > totalMoney:
            ...
            return False
        if bUseBank:
            deltaMoney > self.money and self.b_setBankMoney(self.bankMoney - (deltaMoney - self.money))
            self.b_setMoney(0)
        else:
    ...
    def tossPie(self, x, y, z, h, p, r, sequence, power, timestamp32):
        if self.validate(self.doId, self.numPies > 0, 'tossPie with no pies available'):
            return
    ...
    if simbase.wantKarts:
        ...
        def updateKartDNAField(self, dnaField, fieldValue):
            if checkKartFieldValidity(dnaField):
            ...
            if dnaField == KartDNA.bodyType:
                ...
                accFields = [
                 KartDNA.ebType, KartDNA.spType, KartDNA.fwwType, KartDNA.bwwType, KartDNA.rimsType, KartDNA.decalType]
                colorFields = [
    ...
    if simbase.wantPets:
        ...
        def setSC(self, msgId):
            ...
            if msgId in [21006]:
                ...
                if msgId in [21000, 21001, 21003, 21004, 21200, 21201, 21202, 21203, 21204, 21205, 21206]:
                    self.setHatePets(0)
    ...
    def doBuildingTakeover(self, suitIndex):
        ...
        self.notify.warning('cogTakeOver %s %s %d %d' % (track, level, building.block, self.zoneId))
        return [
    ...
    def tryToUseSpecial(self, special):
        ...
        for curSpecial in self.gardenSpecials:
            if curSpecial[0] == special:
                doIHaveThisSpecial = curSpecial[1] > 0 and True
                break

        if doIHaveThisSpecial:
            return response
        if self.doId == estateOwnerDoId:
        ...
        if estateOwnerDoId:
            ...
            if estate:
                if hasattr(estate, 'avIdList'):
                    ownerIndex = estate.avIdList.index(estateOwnerDoId)
                    ownerIndex >= 0 and estate.doEpochNow(onlyForThisToonIndex=ownerIndex)
