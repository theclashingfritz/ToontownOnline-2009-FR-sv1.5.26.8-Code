...
class DistributedPetAI(DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI, PetBase.PetBase):
    ...
    movieTimeSwitch = {PetConstants.PET_MOVIE_FEED: PetConstants.FEED_TIME, PetConstants.PET_MOVIE_SCRATCH: PetConstants.SCRATCH_TIME, PetConstants.PET_MOVIE_CALL: PetConstants.CALL_TIME}
    movieDistSwitch = {PetConstants.PET_MOVIE_FEED: PetConstants.FEED_DIST.get, PetConstants.PET_MOVIE_SCRATCH: PetConstants.SCRATCH_DIST.get}
    ...
    def __init__(self, air, dna=None):
        ...
        self.gaitFSM = ClassicFSM.ClassicFSM('petGaitFSM', [
         State.State('off', self.gaitEnterOff, self.gaitExitOff), State.State('neutral', self.gaitEnterNeutral, self.gaitExitNeutral), State.State('happy', self.gaitEnterHappy, self.gaitExitHappy), State.State('sad', self.gaitEnterSad, self.gaitExitSad)], 'off', 'off')
        self.gaitFSM.enterInitialState()
        self.unstickFSM = ClassicFSM.ClassicFSM('unstickFSM', [
         State.State('off', self.unstickEnterOff, self.unstickExitOff), State.State('on', self.unstickEnterOn, self.unstickExitOn)], 'off', 'off')
    ...
    def announceZoneChange(self, newZoneId, oldZoneId):
        ...
        if self.estateOwnerId:
            ...
            self.inEstate = 0
            self.estateZones = []
        PetObserve.send(broadcastZones.keys(), PetObserve.PetActionObserve(PetObserve.Actions.CHANGE_ZONE, self.doId, (
         oldZoneId, newZoneId)))
    ...
    def generate(self):
        ...
        if hasattr(self, '_beingCreatedInDB'):
            for i in xrange(len(self.traitList)):
                value = self.traitList[i]
                if value == 0.0:
                    traitName = PetTraits.getTraitNames()[i]
                    traitValue = self.traits.getTraitValue(traitName)
                    DistributedPetAI.notify.info("%s: initializing new trait '%s' to %s, seed=%s" % (self.doId, traitName, traitValue, self.traitSeed))
    ...
    def d_setMovie(self, avId, flag):
        self.sendUpdate('setMovie', [
         flag, avId, ClockDelta.globalClockDelta.getRealNetworkTime()])
    ...
    def handleAvPetInteraction(self, mode, avId):
        if self.avatarInteract(avId):
            ...
            self.movieMode = mode
            callback = {PetConstants.PET_MOVIE_SCRATCH: self.scratch, PetConstants.PET_MOVIE_FEED: self.feed, PetConstants.PET_MOVIE_CALL: self.call}.get(mode)
            if callback is None:
                self.notify.warning('handleAvPetInteraction: Unable to find appropriate Observation Method')
                self.air.writeServerEvent('suspicious', targetId, 'DistributedPetAI: unknown mode: %s' % mode)
                return
    ...
    def __lockPetMoveTask(self, avId):
        ...
        if dist - movieDist > 0.25:
            abs(avgDist - dist) > 0.1 and self.lockMover.move()
