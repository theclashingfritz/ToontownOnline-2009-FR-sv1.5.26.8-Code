...
class LauncherBase(DirectObject):
    ...
    TmpOverallMap = [0.25, 0.25, 0.25, 0.25]
    BANDWIDTH_ARRAY = [
     1800, 3600, 4200, 6600, 8000, 12000, 16000, 24000, 32000, 48000, 72000, 96000, 128000, 192000, 250000, 500000, 750000, 1000000, 1250000, 1500000, 1750000, 2000000, 3000000, 4000000, 6000000, 8000000, 10000000, 12000000, 14000000, 16000000, 24000000, 32000000, 48000000, 64000000, 96000000, 128000000, 256000000, 512000000, 1024000000]
    ...
    def __init__(self):
        ...
        self.ServerVersion = launcherConfig.GetString('server-version', 'no_version_set')
        ...
        self.notify = directNotify.newCategory('LauncherBase')
    ...
    def getProductName(self):
        ...
        productName = 'FR'
        if productName:
            productName = productName != 'DisneyOnline-US' and '_%s' % productName
        else:
            productName = ''
        return productName
    ...
    def addDownloadVersion(self, serverFilePath):
        ...
        if origPath:
            if origPath[(-1)] == '/':
                origPath = origPath[:-1]
            self.fromCD and url.setPath(self.getCDDownloadPath(origPath, serverFilePath))
        else:
    ...
    def download(self, serverFilePath, localFilename, callback, callbackProgress):
        self.launcherMessage(TTLocalizer.LauncherDownloadFile % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
        task = Task(self.downloadTask)
        task.downloadRam = 0
    ...
    def downloadTask(self, task):
        if self.httpChannel.run():
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                if totalBytes:
                    ...
                    self.launcherMessage(TTLocalizer.LauncherDownloadFilePercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': pct})
                else:
                    self.launcherMessage(TTLocalizer.LauncherDownloadFileBytes % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'bytes': bytesWritten})
        ...
        if self.httpChannel.isValid():
            if self.httpChannel.isDownloadComplete():
                bytesWritten = self.httpChannel.getBytesDownloaded()
                totalBytes = self.httpChannel.getFileSize()
                pct = totalBytes and int(round(bytesWritten / float(totalBytes) * 100))
                self.launcherMessage(TTLocalizer.LauncherDownloadFilePercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': pct})
            else:
                self.launcherMessage(TTLocalizer.LauncherDownloadFileBytes % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'bytes': bytesWritten})
    ...
    def downloadMultifile(self, serverFilename, localFilename, mfname, callback, totalSize, currentSize, callbackProgress):
        if currentSize != 0:
            ...
            task.localFilename = localFilename
            task.mfname = mfname
            if currentSize != 0 and task.totalSize == currentSize:
    ...
    def downloadMultifileTask(self, task):
        if self.httpChannel.run():
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(TTLocalizer.LauncherDownloadFilePercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': percentComplete})
            self.foregroundSleep()
            return Task.cont
        statusCode = self.httpChannel.getStatusCode()
        statusString = self.httpChannel.getStatusString()
        self.notify.info('HTTP status %s: %s' % (statusCode, statusString))
        if self.httpChannel.isValid():
            if self.httpChannel.isDownloadComplete() and task.callbackProgress:
                task.callbackProgress(task)
            ...
            if statusCode == HTTPChannel.SCDownloadOpenError or statusCode == HTTPChannel.SCDownloadWriteError:
                ...
                if statusCode == HTTPChannel.SCLostConnection:
                    ...
                    if statusCode == 416 or statusCode == HTTPChannel.SCDownloadInvalidRange:
                        self.httpChannel.getFirstByteRequested() != 0 and self.notify.info('Invalid subrange; redownloading entire file.')
    ...
    def decompressFile(self, localFilename, callback):
        ...
        self.launcherMessage(TTLocalizer.LauncherDecompressingFile % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
        task = Task(self.decompressFileTask)
        task.localFilename = localFilename
        task.callback = callback
        task.lastUpdate = 0
    ...
    def decompressFileTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(TTLocalizer.LauncherDecompressingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': int(round(progress * 100))})
            self.foregroundSleep()
            return Task.cont
        else:
            if errorCode == EUSuccess:
                self.launcherMessage(TTLocalizer.LauncherDecompressingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': 100})
    ...
    def extract(self, mfname, localFilename, destDir, callback):
        ...
        self.launcherMessage(TTLocalizer.LauncherExtractingFile % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
        task = Task(self.extractTask)
        task.mfname = mfname
        task.localFilename = localFilename
        ...
        if task.extractor.setMultifile(task.localFilename):
            self.setPandaErrorCode(6)
    ...
    def extractTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(TTLocalizer.LauncherExtractingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': int(round(progress * 100.0))})
            ...
            if errorCode == EUSuccess:
                self.launcherMessage(TTLocalizer.LauncherExtractingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': 100})
    ...
    def patch(self, patchFile, patcheeFile, callback):
        ...
        self.launcherMessage(TTLocalizer.LauncherPatchingFile % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
        task = Task(self.patchTask)
    ...
    def patchTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(TTLocalizer.LauncherPatchingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': int(round(progress * 100.0))})
            self.foregroundSleep()
            return Task.cont
        else:
            if errorCode == EUSuccess:
                self.launcherMessage(TTLocalizer.LauncherPatchingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': 100})
        ...
        if self.VerifyFiles:
            ...
            for fileDesc in self.ramfile.readlines():
                try:
                    ...
                    serverHash = HashVal()
                    if self.hashIsValid(serverHash, hashStr):
    ...
    def maybeStartGame(self):
        if self.currentPhase >= self.showPhase:
            ...
            self.launcherMessage(TTLocalizer.LauncherStartingGame)
            self.background()
    ...
    def mainLoop(self):
        try:
            ...
            if hasattr(__builtin__, 'base') and getattr(base, 'graphicsEngine', None):
    ...
    def getMultifile(self, mfname):
        ...
        if self.dldb.clientMultifileExists(mfname):
            self.maybeStartGame()
        ...
        if not self.dldb.clientMultifileComplete(mfname) or not self.dldb.clientMultifileDecompressed(mfname):
            if decompressedFilename.exists():
                clientMd5 = HashVal()
                ...
                if clientVer != -1:
                    ...
                    extractedOk and self.notify.info('Multifile appears to have been extracted already.')
    ...
    def patchMultifile(self):
        self.launcherMessage(TTLocalizer.LauncherCheckUpdates % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
    ...
    def patchAndHash(self):
        ...
        if self.PAHNumFiles > 0:
            task = Task(self.patchAndHashTask)
    ...
    def patchAndHashTask(self, task):
        ...
        if os.path.exists(fnameFilename.toOsSpecific()):
    ...
    def decreaseBandwidth(self, targetBandwidth=None):
        if self.DECREASE_BANDWIDTH:
            return 0
        if self.backgrounded:
            if self.everIncreasedBandwidth:
                return 0
            return self.bandwidthIndex == 0 and 0
        else:
            self.bandwidthIndex -= 1
            if targetBandwidth:
